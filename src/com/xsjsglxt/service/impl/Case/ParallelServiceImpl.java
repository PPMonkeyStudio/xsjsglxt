	...
	public page_list_parallelInformationVO VO_Parallelformation_By_PageAndSearch(
		...
		for (xsjsglxt_parallel parallel : listParallel) {
			...
			if (page_list_parallelInformation.getParallel_casename() != null
				...
				parallel.setParallel_casename(
						parallel.getParallel_casename().replaceAll(parallel.getParallel_casename(),
								"<span style='color: #ff5063;'>" + parallel.getParallel_casename().trim() + "</span>"));
			}
			// 2
			if (page_list_parallelInformation.getParallel_person() != null
					&& page_list_parallelInformation.getParallel_person().trim().length() > 0) {
				parallel.setParallel_person(parallel.getParallel_person().replaceAll(parallel.getParallel_person(),
						"<span style='color: #ff5063;'>" + parallel.getParallel_person().trim() + "</span>"));
			}
