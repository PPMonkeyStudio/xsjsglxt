...
public class LostMobilephoneServiceImpl implements LostMobilephoneService {
	...
	public page_list_CasematerialVO VO_LostMobilephoneformation_By_PageAndSearch(
		...
		for (xsjsglxt_lost_mobilephone lost_mobilephone : lost_mobilephoneList) {
			...
			if (page_list_Casematerial.getLost_mobilephone_phone() != null
				...
				lost_mobilephone.setLost_mobilephone_phone(lost_mobilephone.getLost_mobilephone_phone()
						.replaceAll(lost_mobilephone.getLost_mobilephone_phone(), "<span style='color: #ff5063;'>"
								+ lost_mobilephone.getLost_mobilephone_phone().trim() + "</span>"));
			}
			// 2
			if (page_list_Casematerial.getLost_mobilephone_IMEI() != null
					&& page_list_Casematerial.getLost_mobilephone_IMEI().trim().length() > 0) {
				lost_mobilephone.setLost_mobilephone_IMEI(lost_mobilephone.getLost_mobilephone_IMEI()
						.replaceAll(lost_mobilephone.getLost_mobilephone_IMEI(), "<span style='color: #ff5063;'>"
								+ lost_mobilephone.getLost_mobilephone_IMEI().trim() + "</span>"));
			}
			// 3
			if (page_list_Casematerial.getLost_mobilephone_feature() != null
				...
				lost_mobilephone.setLost_mobilephone_feature(lost_mobilephone.getLost_mobilephone_feature().replaceAll(
						lost_mobilephone.getLost_mobilephone_feature(),
	...
	public LostMobilephoneInformationDTO LostMobiephoneInformationOne(xsjsglxt_lost_mobilephone lost_mobilephone) {
		...
		LostMobilephoneInformationDTO lostMobilephoneInformationDTO = new LostMobilephoneInformationDTO(case1, snece,
				lost_mobilephone);
	...
	public boolean remove_Lost_mobilephoneInformationList(List<String> useLost_mobilephoneInformationNumList) {
		...
		for (String lost_mobilephone_id : useLost_mobilephoneInformationNumList) {
			xsjsglxt_lost_mobilephone xsjsglxt_lost_mobilephone = lostMobilephoneDao
					.getLost_mobilephoneByNum(lost_mobilephone_id);
			flag = lostMobilephoneDao
					.deleteLost_mobilephoneById(xsjsglxt_lost_mobilephone.getXsjsglxt_lost_mobilephone_id());// ����
