...
var image_Details = function() {
	...
	if ($(this).text().trim() == "修改") {
		$.post('/xsjsglxt/case/Image_ImageInformationOne', {
			...
			$('#image').find('input[name="image.image_number"]').val(
					xhr_data.image.image_number);
			$('#image').find('input[name="image.image_number"]').text(
					xhr_data.image.xsjsglxt_image_id);
			// 显示模态框
			...
			$('#image .modify_image').unbind().click(
					function() {
						$.post('/xsjsglxt/case/Image_updateImage', $(
								'#image form').serialize()
								+ "&xsjsglxt_image_id=" + image_id, function(
								msg) {
							if (msg == "success") {
								$('#image').modal('hide');
								toastr.info("修改成功!");
		...
		$.post('/xsjsglxt/case/Image_remove_ImageInformationList', formData,
				function(msg) {
					if (msg == 'success') {
						toastr.info('删除成功');
...
function get_ListImageInformationByPageAndSearch(data) {
	$
			.post(
					'/xsjsglxt/case/Image_getAllImageInformation',
					query_data,
					function(xhr_data) {
