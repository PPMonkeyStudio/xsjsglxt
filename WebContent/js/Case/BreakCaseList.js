...
function get_ListBreakecaseInformationByPageAndSearch(data) {
	$
			.post(
					'/xsjsglxt/case/BreakCase_ListBreakecaseInformationByPageAndSearch',
					data,
					function(xhr) {
						...
						for (var len = 0; len < data_list.length; len++) {
							...
							str += '<td><a href="/xsjsglxt/case/Case_page_CaseDetails?id='
									...
									+ '">'
									+ data_list[len].sence.snece_inquestId
									+ '</a></td>';
							str += '<td>'
									+ data_list[len].breakecase.breakecase_type
									+ '</td>';
							str += '<td>'
									+ data_list[len].breakecase.breakecase_according
									+ '</td>';
							str += '<td>'
									+ data_list[len].breakecase.breakecase_capture
									+ '</td>';
							str += '<td>'
									+ data_list[len].breakecase.breakecase_captureUnit
									+ '</td>';
							str += '<td>'
									+ data_list[len].breakecase.breakecase_waitbreakecase
									+ '</td>';
							str += '<td>'
									+ data_list[len].breakecase.breakecase_suspectName
									+ '</td>';
							str += '<td>'
...
var modifi_delete = function() {
	...
	if (type == "修改") {
		$
				.post(
						'/xsjsglxt/case/BreakCase_BreakecaseInformationOne',
						{
						...
						function(xhr_data) {
							...
							str += '<option '
									+ (xhr_data.breakecase.breakecase_type == "新添案件" ? "selected"
											: "") + '>新添案件</option>';
							str += '<option '
									+ (xhr_data.breakecase.breakecase_type == "已有案件" ? "selected"
											: "") + '>已有案件</option>';
							str += '</select></td>';
							str += '<td>嫌疑人姓名</td><td><input style="witdh:70%;" class="form-control" name="breakecase.breakecase_suspectName" type="text" value="'
									+ xhr_data.breakecase.breakecase_suspectName
									+ '"  /></td>';
							str += '</tr>';
							str += '<tr>';
							str += '<td>性别</td><td><input style="witdh:70%;" class="form-control" name="breakecase.breakecase_suspectSex" type="text" value="'
									+ xhr_data.breakecase.breakecase_suspectSex
									+ '"  /></td>';
							str += '<td>出生日期</td><td><input style="witdh:70%;" class="form-control" name="breakecase.breakecase_suspectBirthday" type="text" value="'
									+ xhr_data.breakecase.breakecase_suspectBirthday
									+ '"  /></td>';
							str += '</tr>';
							str += '<tr>';
							str += '<td>身份证号码</td><td><input style="witdh:70%;" class="form-control" name="breakecase.breakecase_suspectIDnum" type="text" value="'
									+ xhr_data.breakecase.breakecase_suspectIDnum
									+ '"  /></td>';
							str += '<td>住址</td><td><input style="witdh:70%;" class="form-control" name="breakecase.breakecase_suspectAddress" type="text" value="'
									+ xhr_data.breakecase.breakecase_suspectAddress
									+ '"  /></td>';
							str += '</tr>';
							str += '<tr>';
							str += '<td>破案依据</td><td><input style="witdh:70%;" class="form-control" name="breakecase.breakecase_according" type="text" value="'
									+ xhr_data.breakecase.breakecase_according
									+ '"  /></td>';
							str += '<td>是否抓获</td><td><input style="witdh:70%;" class="form-control" name="breakecase.breakecase_capture" type="text" value="'
									+ xhr_data.breakecase.breakecase_capture
									+ '"  /></td>';
							str += '</tr>';
							str += '<tr>';
							str += '<td>抓获单位</td><td><input style="witdh:70%;" class="form-control" name="breakecase.breakecase_captureUnit" type="text" value="'
									+ xhr_data.breakecase.breakecase_captureUnit
									+ '"  /></td>';
							str += '<td>带破案件</td><td><input style="witdh:70%;" class="form-control" name="breakecase.breakecase_waitbreakecase" type="text" value="'
									+ xhr_data.breakecase.breakecase_waitbreakecase
									+ '"  /></td>';
							str += '</tr>';
							str += '<tr>';
							str += '<td>备注</td><td  colspan="3"><textarea style="witdh:70%;" class="form-control" name="breakecase.breakecase_remarks">'
									+ xhr_data.breakecase.breakecase_remarks
									+ '</textarea>';
							// 添加存丢失物的id隐藏域(上一兄元素为备注文本域)
							str += '<input name="breakecase.xsjsglxt_breakecase_id" type="hidden" value="'
									+ xhr_data.breakecase.xsjsglxt_breakecase_id
									+ '" />';
							str += '</td>';
							str += '</tr></tbody></table>';
							// str加载到模态框中
						...
						function() {
							...
							$('.query_prompting_info')
									.text(
											'接警时间从'
													+ $(
															'input[name="page_list_senceInformation.start_time"]')
															.val()
													+ '到'
													+ $(
															'input[name="page_list_senceInformation.stop_time"]')
															.val());
						...
						function(xhr) {
							...
							for (var len = 0; len < xhr.SenceInformationDTOList.length; len++) {
								...
								str += '<td>' + data_list.case1.case_address
										+ '</td>';
								str += '<td>'
										+ data_list.case1.case_sonCategory
										+ '</td>';
								str += '<td>'
										+ data_list.case1.case_reporterName
										+ '</td>';
								str += '<td>'
