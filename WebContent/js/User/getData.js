	...
	if(isBack())
		{
			...
			$("#user_case_technology_power_update").val(result.user_case_technology_power);
			$("#user_case_query_power_update").val(result.user_case_query_power);
			$("#user_check_power_update").val(result.user_check_power);
			$("#user_army_manager_power_update").val(result.user_army_manager_power);
			$("#user_technology_manager_power_update").val(result.user_technology_manager_power);
			$("#user_statistics_power_update").val(result.user_statistics_power);
			$("#user_user_manager_power_update").val(result.user_user_manager_power);
...
function loadUserBack()
{
	if (isBack()) {
		...
		for (var i = 0; i < result.list.length; i++) {
			userTable.innerHTML= userTable.innerHTML+"<tr class='trHover'><td>"+result.list[i].user_username+"</td>"+
								"<td>"+result.list[i].user_name+"</td>"+
								"<td>"+result.list[i].user_number+"</td>"+
								"<td>"+result.list[i].user_units+"</td>"+
								"<td>" +
								"<button onclick='deleteUser(this)' value='"+result.list[i].user_id+"' class='btn btn-danger'>删除</button>" +
								"<button onclick='getUserById(this)' value='"+result.list[i].user_id+"' data-toggle='modal' data-target='#updateUser' style='margin-left: 5px;' class='btn btn-primary'>修改</button>" +
								"</td></tr>";
		}
