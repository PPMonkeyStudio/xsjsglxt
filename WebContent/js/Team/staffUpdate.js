							...
							function(key, value) {
								...
								if (key == "xsjsglxt_sex") {
									...
									if (value == "预备党员" || value == "入党积极分子") {
										document
												.querySelector(".staff_appliactionFormTime_label").style.display = "";
										document
												.querySelector(".staff_appliactionFormTime").style.display = "";
										document
												.querySelector(".staff_delevopObjectTime_label").style.display = "";
										document
												.querySelector(".staff_delevopObjectTime").style.display = "";
										$('input[name="policeman.' + key + '"]')
												.val(value);
									} else if (value == "党员") {
										document
												.querySelector(".staff_joinPartyTime_label").style.display = "";
										document
												.querySelector(".staff_joinPartyTime").style.display = "";
										$('input[name="policeman.' + key + '"]')
												.val(value);
									}
								}
								$('input[name="policeman.' + key + '"]').val(
										value);
							});
			$('#staff_isItFormal').val(staff_info.staff_isItFormal);
		...
		if (xmlhttp_work.readyState == 4 && xmlhttp_work.status == 200) {
			...
			if (staff_work == "worksIsNull") {
				...
				for (var len = 0; len < staff_work.length; len++) {
					...
					str2 += '<td>' + staff_work[len].staffWork_address
							+ '</td>';
					str2 += '<td>' + staff_work[len].staffWork_duty + '</td>';
					str2 += '<td>' + staff_work[len].staffWork_startTime
							+ '</td>';
					str2 += '<td>' + staff_work[len].staffWork_stopTime
							+ '</td>';
					str2 += '<td>' + staff_work[len].staffWork_remarks
							+ '</td>';
			...
			if (staff_family == "familyIsNull") {
				...
				for (var len = 0; len < staff_family.length; len++) {
					...
					str3 += '<td >'
							...
							+ '</td>';
					str3 += '<td>' + staff_family[len].staffFamily_name
							+ '</td>';
					str3 += '<td>' + staff_family[len].staffFamily_IDcard
							+ '</td>';
					str3 += '<td>' + staff_family[len].staffFamily_birthday
							+ '</td>';
					str3 += '<td>' + staff_family[len].staffFamily_tel
							+ '</td>';
					str3 += '<td>' + staff_family[len].staffFamily_WeiXin
							+ '</td>';
					str3 += '<td>' + staff_family[len].staffFamily_workSpace
							+ '</td>';
					str3 += '<td>' + staff_family[len].staffFamily_duty
							+ '</td>';
					str3 += '<td>' + staff_family[len].staffFamily_remarks
							+ '</td>';
		...
		if (xmlhttp_move.readyState == 4 && xmlhttp_move.status == 200) {
			...
			if (staff_move == "moveIsNull") {
				...
				for (var len = 0; len < staff_move.length; len++) {
					...
					str4 += '<td>' + staff_move[len].staffMove_outTime
							+ '</td>';
					str4 += '<td>' + staff_move[len].staffMove_guard + '</td>';
					str4 += '<td>' + staff_move[len].staffMove_remarks
							+ '</td>';
			...
			if (staff_reward == "rewardsIsNull") {
				...
				for (var len = 0; len < staff_reward.length; len++) {
					...
					str5 += '<td>' + staff_reward[len].staffReward_situation
							+ '</td>';
					str5 += '<td>' + staff_reward[len].staffReward_Time
							+ '</td>';
		...
		if (xmlhttp_against.readyState == 4 && xmlhttp_against.status == 200) {
			...
			if (staff_against == "principleIsNull") {
				...
				for (var len = 0; len < staff_against.length; len++) {
					...
					str6 += '<td>'
							+ staff_against[len].staffPrinciple_situation
							+ '</td>';
		...
		if (xmlhttp_furlough.readyState == 4 && xmlhttp_furlough.status == 200) {
			...
			if (staff_furlough == "furloughIsNull") {
				...
				for (var len = 0; len < staff_furlough.length; len++) {
					...
					str8 += '<td>'
							+ staff_furlough[len].staffFurlough_startTime
							+ '</td>';
					str8 += '<td>' + staff_furlough[len].staffFurlough_stopTime
							+ '</td>';
					str8 += '<td>'
							+ staff_furlough[len].staffFurlough_whetherStop
							+ '</td>';
					str8 += '<td>'
							+ staff_furlough[len].staffFurlough_mainContent
							+ '</td>';
