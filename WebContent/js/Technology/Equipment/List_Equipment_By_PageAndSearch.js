...
function List_Equipment_By_PageAndSearch(pageIndex) {
	var input_EquipmentSearchText = document.getElementById("input_EquipmentSearchText").value;
		...
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				...
				for (var num = 0; num < equipment_VO.list_xsjsglxt_equipment.length; num++) {
					...
					new_td = document.createElement("td");
					new_tr.appendChild(new_td);
					new_td.innerHTML = '<i  id="'+equipment_VO.list_xsjsglxt_equipment[num].xsjsglxt_equipment_id+'" onClick=EquipmentDetails(this) class="fa fa-pencil-square-o" aria-hidden="true"></i>';
	...
	xhr.open("POST", "/xsjsglxt/Equipment/EquipmentManagement_ListEquipmentByPageAndSearch");
	xhr.send(formData);
