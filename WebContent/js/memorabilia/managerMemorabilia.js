					...
					$
							.confirm({
								...
								content : "<form id='UpdateMemorabiliaForm' name='UpdateMemorabiliaForm'><table class='table table-bordered'>"
										...
										+ "<td>时间：</td><td><input name='memorabilia.memorabilia_time' value='"
										+ jsonData.memorabilia_time
										+ "' class='form-control timeDate' type='text'></td></tr>"
										+ "<tr><td>参与人员：</td><td colspan='3'><input name='memorabilia.memorabilia_join_human' value='"
										+ jsonData.memorabilia_join_human
										+ "' class='form-control' type='text'></td></tr>"
										+ "<tr><td>大事记内容：</td><td colspan='3'><textarea rows='10' class='form-control' name='memorabilia.memorabilia_content'>"
										+ jsonData.memorabilia_content
										+ "</textarea></td><tr>"
										+ "</table></form>",
								buttons : {
									update : {
										...
										action : function() {
											for (var i = 0; i < document.UpdateMemorabiliaForm.elements.length - 1; i++) {
												if (document.UpdateMemorabiliaForm.elements[i].value == "") {
													...
													document.UpdateMemorabiliaForm.elements[i]
															.focus();
													return false;
												}
											}
											var formData = new FormData(
													document
															.getElementById('UpdateMemorabiliaForm'));
											formData
													.append(
															"memorabilia.memorabilia_id",
															dom.id);
											$
													.ajax({
...
var exportMemorabiliaWord = function(dom) {
	window.location = "/xsjsglxt/user/Memorabilia_exportMemorabiliaWord?memorabilia_id="
			+ dom.id;
	return false;
}

var previewMemorabilia = function(dom) {
	$
			.ajax({
				url : '/xsjsglxt/user/Memorabilia_getMemorabiliaById',
				type : 'POST',
				...
				success : function(data) {
					...
					$
							.confirm({
								columnClass : "col-md-12",
								title : "<i class='fa fa-pencil-square-o'></i>预览大事记",
								type : 'yellow',
								content : "<form id='UpdateMemorabiliaForm' name='UpdateMemorabiliaForm'><table class='table table-bordered' style='text-align: center;'>"
										+ "<tr><td style='width:200px'>大事记标题：</td><td style='width:400px'>"
										+ jsonData.memorabilia_title
										+ "</td>"
										+ "<td style='width:200px'>时间：</td><td style='width:400px'>"
										+ jsonData.memorabilia_time
										+ "</td></tr>"
										+ "<tr><td>参与人员：</td><td colspan='3'>"
										+ jsonData.memorabilia_join_human
										+ "</td></tr>"
										+ "<tr><td>大事记内容：</td><td colspan='3'>"
										+ jsonData.memorabilia_content
										+ "</td><tr>" + "</table></form>",
								buttons : {
									close : {
