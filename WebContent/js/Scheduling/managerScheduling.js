...
var updateLog = function(event) {
	...
	$.confirm({
		boxWidth : '500px',
		useBootstrap : false,
		title : '<i class="fa fa-pencil-square-o"></i>修改日志',
		type : 'green',
		content : '<textarea  class="form-control" rows="3" id="updateLogContent"></textarea>',
		buttons : {
			save : {
				text : '修改',
				btnClass : 'btn-blue',
				action : function() {
					if ($('#updateLogContent').val() != '') {
						scheduling['scheduling_log'] = $('#updateLogContent').val();
						var formData = new FormData();
						for (var p in scheduling) {
							formData.append('scheduling.' + p, scheduling[p]);
						}
						$.ajax({
		...
		onContentReady : function() {
			...
			$.ajax({
				...
				success : function(data) {
					var jsonData = JSON.parse(data);
					if (jsonData.scheduling_log) {
						$('#updateLogContent').val(jsonData.scheduling_log);
					}
					scheduling = jsonData;
				}
			});
...
var viewLog = function(event) {
	$.confirm({
		boxWidth : '500px',
		useBootstrap : false,
		title : '<i class="fa fa-pencil-square-o"></i>查看日志',
		type : 'green',
		content : '<textarea  class="form-control" id="updateLogContent"></textarea>',
		buttons : {
			close : {
				text : "关闭",
				btnClass : "btn-red",
